...
Current as of **2026-09-22**, after commit `29bfe91` (live at http://rudeboy.dev/dragonwilds-crafting-atlas/).
...
| 13 | **Isolate depth input (default 3, forward + backward)** *(new)* | ❌ todo | Plan §3, P0-2. |
| 14 | **Switching layout during a running force layout does nothing** *(new bug)* | ❌ todo | Root cause found: `runLayout()` early-returns while `layoutRunning === true`; cola/euler/cose animate up to 4–10 s, so a selection inside that window is silently dropped. Fix in §3, P0-1. |
...
|--------|---------|--------|------------------|
| 01 | Jagex FCP §8.1 attribution line, **verbatim**, in README + app UI | ❌ todo | README has only "© Jagex" credit; the required wording is not present verbatim, and nothing is shown in the app. |
| 02 | Non-affiliation / non-endorsement disclaimer (README + app) | ❌ todo | Not present. |
| 03 | "Non-commercial in perpetuity" statement at top of README | ❌ todo | Not present. |
| 04 | AI-generated disclosure in README | ❌ todo | Not present. |
...
### P0 — broken behavior & the two new TODO items (do next)

- **P0-1 · Fix silent layout-switch drop** *(TODO #14)*
  In `runLayout()`: if a layout is active, call `lay.stop()` on it (all extensions expose `stop()`; for `elk` guard with `lay.stop?.()`), clear `layoutRunning`, then start the new preset. Keep the indicator honest: it should reflect the *new* layout immediately.
  **Verify:** probe-live — start cola, switch to dagre within 1 s, assert indicator label changes and bounds change.

- **P0-2 · Isolate depth control** *(TODO #13)*
  Small number input next to the "Isolate tree" button in the panel (default **3**, min 1, plus an "All" option = current behavior). BFS in both directions, but stop expanding once `depth > limit`. Leaf nodes are always included. Show the applied depth in the toast.
  **Verify:** isolate Iron Bar at depth 1 → only direct neighbors; depth 3 → superset; All → 1,408 nodes (current numbers).
...
- **P1-3 · Verbatim attribution block** *(compliance 01–04)* — add the four P0 texts to README (exact Jagex wording, disclaimer, non-commercial statement, AI disclosure) **and** a small "About" line in the app footer/welcome card with the Jagex sentence. ~30 min, closes four tickets.

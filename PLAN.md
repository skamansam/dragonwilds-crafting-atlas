...
| `node scripts/test-ui.mjs [section]` | **Permanent Playwright UI suite** (serves site/ itself, `--url=` for prod): smoke / panel / undo / layouts sections, pass-fail summary, non-zero exit on failure |
...
| 2026-09-24 | `618eb7e` | **Location checklists** (P4-1 follow-up): `build-location-checklists.mjs` → `docs/checklists/*.md` — per-method checkbox lists (mined/picked/chopped/farmed/caught/chest/dungeon/drops) for items whose `region` is missing, `unknown-source.md` (435 no-annotation items, kind-grouped) + `partial-locations.md` (233); accounting self-check 435 + 233 + 88 = 756 foundable |
| 2026-09-24 | `fc2c33e` | **unknown-source.md sorted by likely in-game progression**: hand-tuned heuristic ladder (metal/zone name tiers → raw gatherables → materials → drop mats → usables → lore/quest) since the dataset carries no level gates; consumer-recipe skills tiebreak; README gains a data-refresh section incl. the hand-annotation loop |
| 2026-09-24 | `d6d4236` | **Legend becomes the full control surface**: **VIEW** section gains **Dead ends** (moved from header), **Possessions** with a live owned-count badge (moved), **Show everything** and **↺ All — reset view** (both relocated; ↺ All now also resets dead ends + possessions focus); retired header chips stay as hidden script-compatible clones (`#ownedCount` id moved into the legend). New **⚙ graph settings** header dropdown (accessible disclosure: aria-expanded, outside-click + Esc close, one-shot listeners) holding **re-layout on graph change** (new, persisted — visibility/isolation/possession changes re-run the layout via a 180ms-debounced scheduler, `runLayout` gains a `reflow:false` gate for explicit requests, boot double-run guarded) and **worker layout** (moved from the header). Help modal + README updated; test-site smoke test updated (legend Food toggle 1,414→1,298); verified: 21-check UI probe (no console errors), `probe-live isolate` + `p12` worker — the `cose-bilkent-tight` battery timeout reproduced byte-identically on the pre-change tree (deterministic live-physics blowup on this box, not a regression) |
| 2026-09-25 | `abeab93` | **Checklists → wide tables**: `build-location-checklists.mjs` rewritten — per-method checkbox files retired; two tables now: `needs-locations.md` (233 method-known, region-missing; known regions pre-ticked) and `unknown-source.md` (435, progression-grouped tables). Columns: Item | Wiki | How it's obtained | every named Ashenfall location in [wiki page order](https://dragonwilds.runescape.wiki/w/Ashenfall) (Brynmoor → The Burning Spire, 34 columns incl. `Ashenfall (whole world)`) | Notes. "How" cells are natural prose mined from cached pages — drop items read "killed from: cows, deer, and wolves" (gather-prose like "gathered from animals…" and small-creature lists recognized; purpose/quest clauses trimmed, `[[Wolf|wolves]]` aliases honored). Deterministic (byte-identical reruns); accounting check intact 435+233+88=756 |
| 2026-09-25 | `190144f` | **Header re-centered**: search, the shown-counts/algorithm line and the Arranging pill form a centered column (`.header-center`, search capped 480px, shrink-safe) that fills the middle and pushes the **⚙ settings + ? help** pair to a right-edge cluster (`.header-right`); the retired header chip tray is hidden wholesale (it was leaving an empty centered slot); mobile breakpoint wraps the center column below the brand. Verified by the prod `--url` run: 28/28, no page errors |
| 2026-09-26 | `a008ced` | **Snapshot controls made findable + reflow actually recomputes**: the 💾/⤓/✕ buttons (previously bare 10px emoji inside the density label — effectively invisible) become a labeled **💾 Save view / ⤓ Export / ✕ Clear** button row in the ⚙ panel with a live status line (your saved view / shared curated view / bundled snapshot / live-computed); row hides entirely on non-snapshot layouts. **Bug fix**: graph-change reflows (legend toggles, isolation, possessions) called runLayout with defaults, which re-applied the static full-graph snapshot — nothing ever moved. scheduleReflow now passes `skipSaved + visibleOnly` (recompute on the shown subgraph, main thread or worker), and a boot-time race is fixed: applyPossessions() at the bottom of app.js scheduled a reflow that fired after the boot saved run started, cancelling it and forcing a full live recompute on every visit; reflows scheduled while a saved run is in flight are dropped unless the hidden-node count actually changed (class-based, since :visible lags the style pass). test-ui.mjs grows to 35 checks (save→reload→clear round-trip, reflow-moves-Furnace regression) — 35/35 green |
| 2026-09-26 | (this commit) | **Header middle un-stacked + Arranging pill moves onto the graph**: `.header-center` becomes a horizontal row — search and the shown-counts/algorithm line sit side by side (search keeps its 480px preferred width but shrinks fluidly instead of being pinned); the gold **Arranging… pill leaves the header entirely** and becomes a stage overlay (absolute, top-center, z-30, styled after the pathbar) so layout progress is visible where the graph actually is; the pill label drops its redundant "Arranging…" suffix; mobile breakpoint keeps the center as a full-width row. Plus **scripts/checklist-csv.mjs**: round-trips the wide hand-annotation checklists through CSV (RFC-4180, CRLF) so they can be edited in a spreadsheet instead of a 38-column markdown table — `node scripts/checklist-csv.mjs unknown-source` exports, `--merge` folds edits back prettier-aligned; a blank "How" cell means keep the generator's prose; wiki cells are repaired/normalized against site/data.json on merge (data.json authoritative — caught and fixed 17 URLs truncated at embedded parens, e.g. Desert_Weed_(plant)); merge is idempotent (0 rows on rerun); unknown-source.csv generated with all 20 hand-filled answers intact. Suite 35/35 |\n
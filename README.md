...
- **saved positions**: layouts with a precomputed snapshot (currently *elk-layered* and
  *cose-bilkent*, in `site/layouts/manifest.js`) apply instantly — the algorithm line shows
  **· saved**. Turn it off to always recompute live. Snapshots are generated by
  `scripts/gen-layouts.mjs`, and Desktop-authored positions can be added to the manifest
  in the same shape.
